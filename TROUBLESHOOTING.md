...
| Import fails: `these files would undo settings saved after your last import …` | `add-source`, `restore`, or a `--merge` / `seed_companies.py` script changed settings in the database since your last import, and your files still have the old values. Importing them would silently drop those changes, so import refuses, names each setting, and writes nothing. | Export the settings in effect (`docker compose run --rm web python -m src.settings export /data/export`), bring the listed settings into your files from `./data/export/`, then import again — it succeeds once the files carry them. To discard those changes on purpose, re-run the import with `--force`. |
