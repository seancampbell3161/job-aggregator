...
| Symptom | Likely cause | Fix |
|---|---|---|
| `docker pull` fails with **404** or **denied** | The GHCR package is private (its default on first publish), or the tag doesn't exist. | Check `docker buildx imagetools inspect ghcr.io/seancampbell3161/job-aggregator:latest` — if that also fails, the package needs to be made public in its GHCR settings; if it succeeds, double-check the tag you asked for. |
| Files under `./data` are root-owned | The container was created from an image older than v0.12.0, before the entrypoint took over fixing ownership. | Pull a current image and recreate — the entrypoint chowns `./data` to the app user on first boot. If it can't (`docker compose logs web` shows a chown warning), the mount is read-only or networked; fix permissions by hand. |
| First `docker compose up -d` (or `--build` in a clone) takes a while | Expected — the image bundles WeasyPrint's native libraries (and Chromium, on the `-headless` tag), so the first pull or build is the largest part. | Wait it out; later starts reuse the cached image/layers. |
...
| Local Ollama provider, but scoring never happens | Started the stack without the Ollama profile, or didn't pull the model. | `docker compose --profile ollama up -d` (add `--build` in a clone) then `docker compose exec ollama ollama pull <model>`. |
...
| An **Avature** board never polls / the `headless` tier never runs | No `sources.avature` entries yet, or you're on the default (slim) image, which has no browser by design. | Add a board (Getting Started → Avature). `docker compose logs poller \| grep headless_unavailable` confirms the slim-image case — switch both services to `ghcr.io/seancampbell3161/job-aggregator:latest-headless` (or a pinned `-headless` tag) and `docker compose pull && docker compose up -d`; in a clone, add `target: headless` under both `build:` keys in `docker-compose.override.yml` (see the comment at the bottom of that file) and `docker compose up -d --build` instead. **Phenom** boards are fetched over plain HTTP and are unaffected. |
| **Automated board discovery** finds nothing | It fingerprints only a *budget* of seeds per day — matches join `ats` polling over ~8 days, not at once. | Watch the `board_discovery_done` log for `matched > 0`. If `swept` is `0` every cycle, the seed CSV is missing from the image — `docker compose pull && docker compose up -d` (a clone: `--build`) to pick up a current one. |
...
| Symptom | Likely cause | Fix |
|---|---|---|
| `/kit` is empty or has no copy buttons | No `kit_facts` document exists yet — nothing has been scaffolded, hand-written, or imported. | Draft your structured résumé at `/settings/documents/draft` and save it — saving scaffolds `kit_facts` for you if you don't already have one. Or hand-write it: `cp resume/facts.example.yaml resume/facts.yaml`, edit it, then re-import (or paste/edit at `/settings/documents?kind=kit_facts`) — `/kit` shows it on the next refresh. |
| The `/kit` "Apply Autofill" bookmarklet does nothing | It's dragged to the bookmarks bar but clicked on a non-supported form, or your `kit_facts` changed since it was last dragged. | It autofills Greenhouse / Lever / Ashby forms only. Re-drag it from `/kit` after your facts change (a fresh save, a re-scaffold, or a re-import). |
| Gmail badges never appear on `/board` | 2-Step Verification off, wrong app password, or `.env` not re-read. | Generate a Gmail **app password** (requires 2FA), then `python -m src.settings set-secret gmail_app_password` (and `gmail_address`) — applies live, no recreate — or set `JOB_AGG_GMAIL_ADDRESS` + `JOB_AGG_GMAIL_APP_PASSWORD` in `.env` and `docker compose up -d` (Compose re-reads `.env` at container recreation). |
| Matches is empty though alerts fired | It shows only postings notified **after** display data started being saved — the pipeline persists display fields at notify time; older rows are id+score+gaps only. | New matches populate it automatically. |
...
| Tailor deep-link in an alert doesn't work | `tailor_endpoint_url` unset, the URL isn't reachable from your phone, or the `resume_content` document tailoring reads is missing. | Set `tailor_endpoint_url` (`set-secret tailor_endpoint_url`, or `JOB_AGG_TAILOR_ENDPOINT_URL` in `.env`) to a reachable URL (LAN IP or Tailscale/Cloudflare Tunnel) — the signing secret is generated automatically; draft `content.json` at `/settings/documents/draft`, hand-write it, or import it (see `resume/README.md`) — an evidence bank is optional. |
