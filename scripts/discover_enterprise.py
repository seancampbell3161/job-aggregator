...
Usage:
  ...
  uv run python scripts/discover_enterprise.py --merge          # save matched entries into settings

Dry-run first, review the report, then re-run with --merge. Merge saves
validated entries into the settings (one new version, deduped against
existing sources + discovered slugs + suppressed connectors) and applies
live — no restart. Each enterprise board adds roughly 2-5s to the ats cycle
— after merging a large batch, check /pipeline cycle duration before merging
more."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = open_service() if args.merge else None
    ...
    if service is not None:
        added, summary = merge_results_into_settings(
            service, results, label="discover_enterprise", already_polled=_store_names_fail_soft(),
        )
        if added:
